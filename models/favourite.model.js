...
static async markFavourite(user_id ,video_id,language_type) {
  const [rows] = await db.query(
    `INSERT INTO tbl_favorites_video (video_id_FK, user_id_FK,language_type) VALUES (?, ?,?)`,
    [video_id, user_id,language_type]
  );
  return rows.insertId;
}
static async isFavouriteExist(user_id ,video_id,language_type){
    const[rows]=await db.query('SELECT * from tbl_favorites_video where video_id_FK=? and language_type=? and user_id_FK=?',[video_id,language_type,user_id]);
    return rows[0];
}
