...
class Notes{
    static async upload({chapter_id,pdf_title,pdf_url,is_paid}){
        const [insert]=await db.query(`insert into tbl_chapter_notes (chapter_id_FK	,pdf_title,	pdf_url,is_paid) values(?,?,?,?)`,[chapter_id,pdf_title,pdf_url,is_paid]);
...
static async adNotifictionInDb({ className, subjectName, chapterName, pdf_title }) {
  const [rows] = await db.query(
    ...
    [className, subjectName, chapterName, pdf_title,'Notes'] 
  );
